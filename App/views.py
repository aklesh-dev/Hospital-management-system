...
# --cache_control to distroy session
...
def add_patient(request):
    if request.method == "POST":
        if request.POST.get('name') and request.POST.get('phone') and request.POST.get('email') and request.POST.get('age') and request.POST.get('gender') or request.POST.get('note'):
            ...
            messages.success(request, "Failed to add patient !!!")
    else:
...
def edit_patient(request):
    if request.method == "POST":
        ...
        if patient != None:
            ...
            messages.success(request, 'Patient not available !!!')
...
def delete_patient(request, patient_id):
    patient = Patient.objects.get(id = patient_id)
    patient.delete()
    messages.success(request, "Patient remove successfully.")
    return HttpResponseRedirect('/backend')